...
Joe Militello
9/26/16
This is the edited matrixLED.js I added the yellow, red, and green features.
*/
...
//Find if it is green, red, or yellow and change approrpiately
     if((disp[i]==(disp[i]|0x1<<j))&&(dispRed[i]==(dispRed[i]|0x1<<j))){
          //dispRed[i] ^= 0x1<<j;
          disp[i] ^= 0x1<<j;;
      }
     else if((dispRed[i]==(dispRed[i]|0x1<<j))){
          dispRed[i] ^= 0x1<<j;
    }else  if((disp[i]==(disp[i]|0x1<<j))){
          dispRed[i] ^= 0x1<<j;
          //disp[i] ^= 0x1<<j;
    }else{
	      disp[i] ^= 0x1<<j;
     }
     //Send red and green byte
    ...
    //Added Turning on Yellow, Red, and Green
    ...
    */
	...
	var i, j;
        ...
        for (i = 0; i < disp.length; i++) {
            ...
            //Added the ability to see that it is red, yellow, or green
            for (j = 0; j < 8; j++) {
                  if((disp[i]>>j&0x1 === 1)&&(dispRed[i]>>j&0x1 === 1)){
                        $('#id'+i+'_'+j).addClass('onYellow');
                  } else {
